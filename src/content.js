...
(function () {
  ...
  function injectCompareButton(body, metadata) {
    document.getElementById('pg-compare-btn')?.remove();

    // 개발자 모드일 때만 표시
    safeStorageGet(['devMode'], (data) => {
      if (!data.devMode) return;
      _renderCompareButton(body, metadata);
    });
  }
  ...
  function showComparisonPanel(comparison, metadata) {
    ...
    panel.innerHTML = `
      <div style="padding:14px 16px;display:flex;align-items:center;justify-content:space-between;
        ...
        <button id="pg-compare-close" style="background:none;border:none;cursor:pointer;
                color:${T_text2};font-size:16px;padding:0 4px">✕</button>
      </div>
      <table style="width:100%;border-collapse:collapse">
        <thead>
          <tr style="background:${T_bg2}">
            <th style="padding:8px 12px;text-align:left;font-size:11px;color:${T_text2};font-weight:500">모델</th>
            <th style="padding:8px 12px;text-align:left;font-size:11px;color:${T_text2};font-weight:500">위험도</th>
            <th style="padding:8px 12px;text-align:left;font-size:11px;color:${T_text2};font-weight:500">신뢰도</th>
            <th style="padding:8px 12px;text-align:left;font-size:11px;color:${T_text2};font-weight:500">응답속도</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
      <div style="padding:10px 16px;border-top:1px solid ${T_border};display:flex;align-items:center;justify-content:space-between">
        <span style="font-size:11px;color:${T_text2}">AI 모델별 판단은 다를 수 있습니다</span>
        <button id="pg-compare-export" style="background:none;border:1px solid ${T_border};border-radius:6px;
                padding:4px 10px;font-size:11px;color:${T_text2};cursor:pointer">JSON 내보내기</button>
      </div>`;

    document.body.appendChild(panel);
    panel.querySelector('#pg-compare-close')?.addEventListener('click', () => panel.remove());

    panel.querySelector('#pg-compare-export')?.addEventListener('click', () => {
      safeStorageGet(['compareResults'], (data) => {
  ...
  function showPanel(status, data, metadata) {
    ...
    // 안전한 항목(flagged:false)은 처음부터 체크
    // 위험한 항목(flagged:true)은 빈 체크박스 — 사용자가 직접 확인 후 체크
    const checkboxes = panel.querySelectorAll('.pg-check');
    if (status === 'result' && data?.checklist) {
      data.checklist.forEach((item, i) => {
        const cb = checkboxes[i];
        if (!cb) return;
        cb.checked = !item.flagged;
        const wrap = cb.closest('.pg-checklist-item');
        if (wrap) wrap.style.opacity = (!item.flagged) ? '0.45' : '1';
      });
    }
  ...
  function buildHTML(status, data, metadata) {
    ...
    const checklist = (r.checklist || []).map((item, i) => {
      ...
      const tc = flagged ? T.red   : T.green;
      const cc = flagged ? T.red   : T.green;
      const ib = flagged
      ...
      const itemOpacity = flagged ? 'opacity:0.55;' : '';
      return `
        <div style="${ib};border-bottom:1px solid ${T.borderLight};${itemOpacity}" class="pg-checklist-item">
          <div style="display:flex;align-items:flex-start;gap:8px">
            <label style="display:flex;align-items:flex-start;gap:8px;cursor:pointer;flex:1">
              <input type="checkbox" class="pg-check" ${flagged ? 'checked' : ''}
    ...
    return wrap(`
      <div>
        ...
        <div style="padding:10px 16px;border-top:1px solid ${T.borderLight};
          ...
          위 체크리스트는 <strong>AI가 자동으로 판별</strong>한 결과입니다.<br>
          오탐 가능성이 있으므로 <strong>사용자께서 한 번 더 확인</strong>해 주시기 바랍니다.<br>
          <span style="opacity:0.7">최종 판단의 책임은 사용자에게 있습니다 · PhishGuard AI</span></div>
      </div>`);
  }
